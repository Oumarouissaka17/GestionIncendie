...
def details_dref(request):
    # Calculs et statistiques sur les données
    # --------- Sous directions -----------------
    ...
    surf_par_icd_current = ""
    if nbr_icd_curent_year > 0:
        if srf_current_year is not None:
    ...
    if nbr_total_incendies > 0:
        if surface_total_brl is not None:
    ...
    context = {
        # Variables contenant les résultats des calculs et des graphiques
